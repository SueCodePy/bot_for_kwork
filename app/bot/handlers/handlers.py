...
from app.bot.keyboards.inline_keyboard import next_kwork_keyboard, show_first_kwork_keyboard
...
async def viewing_data(call: CallbackQuery):
...
@router.callback_query(F.data == 'next_data')
async def show_new_kwork(call: CallbackQuery):
    user_id = call.from_user.id
    kwork = await show_kwork(user_id)
